...
Deno.serve(async (req) => {
    try {
        ...
        if (!id || !resume_url) {
            return new Response(JSON.stringify({ error: 'missing id or resume_url' }), { status: 400 });
        }
...
<head>
  ...
  <style>
    ...
    .button { display: inline-block; background-color: ${GOLD}; color: #ffffff !important; padding: 14px 28px; text-decoration: none; border-radius: 4px; font-weight: bold; font-size: 14px; text-transform: uppercase; letter-spacing: 1px; transition: background-color 0.3s; }
